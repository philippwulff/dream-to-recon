...
from configs.structured_configs.data_config import DataConfigWaymo
...
class WaymoDataset(Dataset):
    ...
    @classmethod
    def from_conf(cls, conf: DataConfigWaymo, **kwargs):
        return cls(
    ...
    def load_depth(self, seq, id):
        ...
        depth[
            ((points_cam[:, 1] * .5 + .5) * self.target_image_size[0]).astype(np.int32).clip(0, self.target_image_size[0] - 1),
            ((points_cam[:, 0] * .5 + .5) * self.target_image_size[1]).astype(np.int32).clip(0, self.target_image_size[1] - 1)] = points_cam[:, 2]
    ...
    def __getitem__(self, index: int):
        ...
        #     projs[i][..., 0, 0] = (projs[i][..., 0, 0] / self.target_image_size[1]) * 2
        #     projs[i][..., 1, 1] = (projs[i][..., 1, 1] / self.target_image_size[0]) * 2
        #     projs[i][..., 0, 2] = (projs[i][..., 0, 2] / self.target_image_size[1]) * 2 - 1
        #     projs[i][..., 1, 2] = (projs[i][..., 1, 2] / self.target_image_size[0]) * 2 - 1
