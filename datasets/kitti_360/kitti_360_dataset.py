...
from configs.constants.constants import DEPTH_NORM_CONST_IN_PNG

from datasets.kitti_360.annotation import KITTI360Bbox3D
from utils.augmentation import get_color_aug_fn
from configs.structured_configs.data_config import DataConfigKitti360
...
class Kitti360Dataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        # Keep the rectified img size
        self.im_size_resized = None
    ...
    def process_img(self, img: np.array, color_aug_fn=None, resampler:FisheyeToPinholeSampler=None):
        if resampler is not None and not self.is_preprocessed:
            ...
            if self.target_image_size:
                im_h, im_w = self._calibs["im_size"]
                self.im_size_resized = (im_h * self.target_image_size[0]/img.shape[0], im_w * self.target_image_size[1]/img.shape[1])
    ...
    def load_predicted_depth(self, seq, img_id, is_right):
        """Reads the depth from a .pth file with a single image of shape [H, W]."""
        filepath = os.path.join(self.predicted_depth_path, f"{seq}__{img_id}__{'r' if is_right else 'l'}.png")
        depth = io.load_array(filepath) * DEPTH_NORM_CONST_IN_PNG
        
        depth = cv2.resize(depth, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_LINEAR)
        
        return torch.tensor(depth[None, :, :])
    ...
    def __getitem__(self, index: int):
        ...
        # The ids in this dataset
        ids = [id] + [max(min(i, seq_len-1), 0) for i in range(id - self._left_offset, id - self._left_offset + self.frame_count * self.dilation, self.dilation) if i != id]
        ids_fish = [max(min(id + self.fisheye_offset, seq_len-1), 0)] + [max(min(i, seq_len-1), 0) for i in range(id + self.fisheye_offset - self._left_offset, id + self.fisheye_offset - self._left_offset + self.frame_count * self.dilation, self.dilation) if i != id + self.fisheye_offset]
        # The ids in the format of the underlying KITTI dataset
        ...
        if self.return_predicted_depth:
            predicted_depths = [self.load_predicted_depth(sequence, ids[0], is_right)]
        else:
            predicted_depths = []
        ...
        _proc_time = torch.tensor(time.time() - _start_time)

        # print(_loading_time, _processing_time, _proc_time)

        data = {
            "imgs": imgs,                               # list of [3, H, W]       
            "projs": projs,                             # list of [3, 3] 
            "poses": poses,                             # list of [3, 3]
            "depths": depths,                           # list of [1, H, W]
            "predicted_depths": predicted_depths,       # list of [1, H, W]
            # "hw_unnorm": torch.tensor([376, 1408]),
            # "hw_unnorm": torch.tensor((self.target_image_size[1], self.target_image_size[0])),
            # "hw_unnorm": self.im_size_rect_resized,
            "hw_unnorm": torch.tensor(self._calibs["im_size"] if self.im_size_resized is None else self.im_size_resized),
            # "hw_unnorm": torch.tensor(imgs[0].shape[1:]),
            "ts": ids,
            "3d_bboxes": bboxes_3d,
            "segs": segs,
            "t__get_item__": torch.tensor([_proc_time]),
            "idxs": torch.tensor([index])
        }
    ...
    @classmethod
    def from_conf(cls, conf: DataConfigKitti360, **kwargs):
        return cls(
