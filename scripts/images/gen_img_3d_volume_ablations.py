...
def main():
    ...
    config = load_and_setup_config(
        ...
        # config_name="exp_recon_no_density_loss", 
    )
    dataset, out_path = setup_task(config, "figures/3d_volume_ablations", return_dataset="test")
    ...
    model_name = "full"
    # no uncert
    # config.BTS.RESUME_FROM = "/home/stud/wph/storage/user/BTS/out/recon/EXP335_backend-gloo-4_run_0/best_model.pt"
    # model_name = "no_uncert"
    # no density loss
    config.BTS.RESUME_FROM = "/home/stud/wph/storage/user/BTS/out/recon/EXP336_backend-gloo-4_run_0/best_model.pt"
    model_name = "no_density_loss"
    # no depth loss
    config.BTS.RESUME_FROM = "/home/stud/wph/storage/user/BTS/out/recon/EXP337_backend-gloo-4_run_0/best_model.pt"
    model_name = "no_depth_loss"
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            match idx:
                case 44:
                    in_img_left_crop = w // 2
                    scene_camera_eye = [0.8, -0.3, -0.8]
                case 72:
                    in_img_left_crop = w // 6
                    scene_camera_eye = [0.7, -0.5, 0.4]
                    bbox = (0.25, 0.05, 0.8, 0.45)
                case 228:
                    in_img_left_crop = w // 2
                    scene_camera_eye = [-0.7, -0.9, 0.5]
                case 266:
                    in_img_left_crop = w // 2
                    scene_camera_eye = [0.7, -0.5, -0.9]
                    bbox = (0.2, 0.5, 0.9, 1)
                case 368:
                    in_img_left_crop = w // 10
                    scene_camera_eye = [0.6, -0.4, 0.4]
                    bbox = (0.13, 0.15, 0.6, 0.45)
                case _:
                    in_img_left_crop = 0
                    in_img_w = w
                    scene_camera_eye = [1.2, -1.2, 0]

            fig = make_3d_fig("", up=up, eye=scene_camera_eye, showgrid=False, transparent_plot_bg=True, width=400, height=250)
            ...
            if not DRY_RUN:
                filepath = os.path.join(out_path, f"ablation_{idx:03d}_{model_name}")
                img_path = os.path.join(out_path, f"input_{idx:03d}_ablation.png")
                
                print(f"Saving to {img_path} and {filepath}.png")
                save_image(in_img[:, :, in_img_left_crop:in_img_left_crop + in_img_w], img_path)


                progress_bar.set_postfix_str("Saving PNG")
                fig.write_image(filepath + ".png", scale=dpi2scale(500, width_in_inches=PAGE_WIDTH_INCHES/9, dpi=500))
                ...
                if bbox is not None:
                    ...
                    x0, y0, x1, y1 = bbox
                    bbox_resized = (x0 * image_width, y0 * image_height, x1 * image_width, y1 * image_height)
                    draw.ellipse(bbox_resized, outline="red", width=5)
