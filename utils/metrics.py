...
class MeanMetric(Metric):
    ...
    def update(self, value):
        if (
            (isinstance(value, torch.Tensor) and (torch.any(torch.isnan(value)) or torch.any(torch.isinf(value)))) or 
            (isinstance(value, (int, float)) and (torch.any(torch.isnan(torch.tensor(value))) or torch.any(torch.isinf(torch.tensor(value)))))
            ):
            return
        self._sum += value
...
class VarianceMetric(Metric):
    def __init__(self, output_transform=lambda x: x["output"], device="cpu"):
        ...
        self._num_examples = None
        self.required_output_keys = ()
        super(VarianceMetric, self).__init__(output_transform=output_transform, device=device)

    @reinit__is_reduced
    def reset(self):
        self._sum = torch.tensor(0, device=self._device, dtype=float)
        self._sum_squared = torch.tensor(0, device=self._device, dtype=float)
        self._num_examples = 0
        super(VarianceMetric, self).reset()
    ...
    @sync_all_reduce("_num_examples:SUM", "_sum:SUM", "_sum_squared:SUM")
    def compute(self):
        if self._num_examples == 0:
            raise NotComputableError('CustomAccuracy must have at least one example before it can be computed.')
        mean = self._sum.item() / self._num_examples
        variance = (self._sum_squared.item() / self._num_examples) - (mean ** 2)
        return variance
