...
import bts.common.util as util


class RaySampler:
    def sample(self, images, poses, projs):
        """Samples rays with per-ray GT pixel."""
        raise NotImplementedError

    def reconstruct(self, render_dict):
        """Reconstructs maps from a rendered batch of rays."""
        raise NotImplementedError
...
class ImageRaySampler(RaySampler):
    ...
    def sample(self, images, poses, projs):
        ...
        if self.height is None or images is not None:
    ...
    def reconstruct(self, render_dict, channels=None):
        ...
        if channels is None:
            channels = self.channels    # Number of channels in rendered imgs

        c_rgb = coarse["rgb"]  # n, n_pts, v * 3
        c_depth = coarse["depth"]
        c_depth_var = coarse["depth_var"]
        c_invalid = coarse["invalid"]

        f_rgb = fine["rgb"]  # n, n_pts, v * 3
        f_depth = fine["depth"]
        f_depth_var = fine["depth_var"]
        f_invalid = fine["invalid"]
        
        # c_n_smps = c_weights.shape[-1]
        # f_n_smps = f_weights.shape[-1]
        # TODO keep an eye on whether this is correct
        c_n_smps = c_invalid.shape[-2]
        f_n_smps = f_invalid.shape[-2]

        n, n_pts, v_c = c_rgb.shape
        v_in = n_pts // (self.height * self.width)
        v_render = v_c // channels      # Number of images/pixels used for rendering.
        # (This can be a different v from the sample method)
        ...
        coarse["depth_var"] = c_depth_var.view(n, v_in, self.height, self.width)
        ...
        fine["depth_var"] = f_depth_var.view(n, v_in, self.height, self.width)
        fine["invalid"] = f_invalid.view(n, v_in, self.height, self.width, f_n_smps, v_render)
        
        if "weights" in coarse:
            c_weights = coarse["weights"]
            f_weights = fine["weights"]
