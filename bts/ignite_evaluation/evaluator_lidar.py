...
from bts.ignite_evaluation.evaluator import get_dataflow, InferenceWrapper
from utils.metrics import MeanMetric, VarianceMetric
...
from utils.plotting import render_profile, plot_profile, color_tensor, cmap_magma, set_spines, set_thesis_rcparams, PAGE_WIDTH_INCHES
from matplotlib.patches import Rectangle
from bts.gt_synthesis.gt_synthesis import Outputs
...
def get_pts(x_range, y_range, z_range, ppm, ppm_y, y_res=None, specify_yslice=None):
    ...
    xyz = torch.stack((x, y, z), dim=-1)    # [y_res, z_res, x_res, 3]
...
class LidarOccupancyWrapper(InferenceWrapper):
    def __init__(
        self, 
        dataset: Kitti360Dataset | WaymoDataset, 
        mode: Literal["recon", "cascade", "depth_pred"] = "recon",
        query_batch_size: int = 50000,
        occ_threshold: float = 0.5,
        x_range: Tuple[float, float] = [-4, 4],
        y_range: Tuple[float, float] = [0, 0.75],
        z_range: Tuple[float, float] = [20, 4],
        ppm: int = 10,
        ppm_y: int = 4,
        y_res: int = 1,
        specify_yslice = None,
        cut_far_invisible_area: bool = False,
        save_gt_occ_map_path: str = "",
        read_gt_occ_path: str = "",
        is_eval_object: bool = False,
        read_gt_obj_path = None,
        obj_x_expand: float = 0.,
        obj_z_expand: float = 0.,
        ...
        **kwargs,
        ...
        self.aggregate_timesteps = gt_aggregate_timesteps

        self.z_near = self.cfg.BTS.MODEL_CONF.z_near
        self.z_far = self.cfg.BTS.MODEL_CONF.z_far
        self.query_batch_size = query_batch_size
        self.occ_threshold = occ_threshold

        # 3D volume used for the evaluation
        ...
        self.ppm_y = ppm_y
        self.y_res = y_res
        self.specify_yslice = specify_yslice
        ...
        self.cut_far_invisible_area = cut_far_invisible_area
        self.save_gt_occ_map_path = save_gt_occ_map_path
        self.read_gt_occ_path = read_gt_occ_path

        # Objects-level evaluation settings
        self.is_eval_object = is_eval_object
        self.read_gt_obj_path = read_gt_obj_path
        self.obj_z_expand = int(obj_z_expand * self.ppm)
        self.obj_x_expand = int(obj_x_expand * self.ppm)
        ...
        self.count = 0
        print(f"self datalen:{len(self.dataset)}")
        print(f"x_range:{x_range}, y_range:{y_range}, z_range:{z_range}, ppm:{ppm}, ppm_y:{ppm_y}, y_res:{y_res}, aggregate_timesteps:{self.aggregate_timesteps}")
    ...
    def forward(self, data):
        ...
        match self.mode:
            case "depth_pred":
                ...
                net = self.gt_synthesizer.renderer.net
            case "recon":
                data = super().forward(data, forward_type="recon")
                net = self.renderer.net
            case "cascade":
                # This mode can also be used to eval the controlnet using a single cascade step.
                data = super().forward(data, forward_type="controlnet_cascade")
                net = self.cascade_model.gt_synthesizer.renderer.net
                # We use the net as it was encoded for the last step. 
                # This means that the last step is not taken into account during this evaluation.
            case _:
                raise ValueError(f"Evaluation mode {self.mode} if not available.")
        
        images = torch.stack(data["imgs"], dim=1)                           # n, v, c, h, w
        poses = torch.stack(data["poses"], dim=1)                 # n, v, 4, 4 w2c
        index = data["idxs"].item()
        ...
        # If it is a subset.
        dataset = self.dataset.dataset if hasattr(self.dataset, "dataset") else self.dataset

        if isinstance(dataset, Kitti360Dataset):
            seq, id, _ = dataset._datapoints[index]
            seq_len = dataset._img_ids[seq].shape[0]
            ...
            seq, id = dataset._datapoints[index]
            seq_len = len(dataset._poses[seq])
            cam_incl_adjust = torch.eye(4).view(1, 1, 4, 4)
        else:
        ...
        if self.read_gt_occ_path == "":
            if isinstance(dataset, Kitti360Dataset):
                T_velo_to_pose = torch.tensor(dataset._calibs["T_velo_to_pose"], device=device)

                # NOTE: points_all contains xxxxx frame point clouds
                for id in range(id, min(id + self.aggregate_timesteps, seq_len)):
                    points = np.fromfile(os.path.join(dataset.data_path, "data_3d_raw", seq, "velodyne_points", "data", f"{self.dataset._img_ids[seq][id]:010d}.bin"), dtype=np.float32).reshape(-1, 4)
                    points[:, 3] = 1.0
                    points = torch.tensor(points, device=device)
                    # NOTE also transfer velo coods to cam0
                    velo_pose = world_transform.squeeze() @ torch.tensor(dataset._poses[seq][id], device=device) @ T_velo_to_pose
                    points_all.append(points)
                    velo_poses.append(velo_pose)
            elif isinstance(dataset, WaymoDataset):
                bbox_path = f"data/waymo_bbox_out/{seq}/lidar_box/{init_id:010d}.npy"
                bboxes = np.load(bbox_path)

                for id in range(id, min(id + self.aggregate_timesteps, seq_len)):
                    points = np.load(os.path.join(dataset.data_path, seq, "lidar", f"{id:010d}.npy"))
                    points = torch.tensor(points, device=device)
                    points = torch.cat((points, torch.ones_like(points[:, :1])), dim=-1)
                    # The lidar points should already be in the camera 1 (front) coordinate system!
                    lidar_2_world = world_transform.squeeze() @ torch.tensor(dataset._poses[seq][id], device=device)
                    points_all.append(points)
                    velo_poses.append(lidar_2_world)

                    # Visualization
                    # pw = (lidar_2_world @ points.T).T
                    # plt.scatter(pw[0, :].cpu(), pw[1, :].cpu(), s=1)
                    # init_pose = torch.eye(4, device=device)
                    # plt.scatter(init_pose[0, 3].cpu(), poses[1, 3].cpu(), c='r')
                    # plt.savefig("temp.png", dpi=300)

            velo_poses = torch.stack(velo_poses, dim=0)
        else:
            if isinstance(dataset, Kitti360Dataset):
                name = "{}_{:010d}".format(seq, dataset._img_ids[seq][init_id])
            elif isinstance(dataset, WaymoDataset):
                name = "{}_{:010d}".format(seq, init_id)
        ...
        for i_from in range(0, len(q_pts), self.query_batch_size):
            ...
            q_pts_h = torch.cat((q_pts_, torch.ones_like(q_pts_[:, :1])), dim=-1)
            q_pts_h = (cam_incl_adjust.squeeze().inverse().to(q_pts_h) @ q_pts_h.mT).mT
            q_pts_ = q_pts_h[:, :3]
            
            _, _, densities_, _ = net(q_pts_.unsqueeze(0), only_density=True)
        ...
        if self.read_gt_occ_path == "":
            ...
            if isinstance(dataset, WaymoDataset):
                # Add occupancy information from the bounding boxes      
                is_occupied = is_occupied.reshape(yd, zd, xd)
                is_visible = is_visible.reshape(yd, zd, xd)
                for xi in range(xd):
                    for yi in range(yd):
                        for zi in range(zd):
                            x = np.linspace(self.x_range[0], self.x_range[1], xd)[xi]
                            y = np.linspace(self.y_range[0], self.y_range[1], yd)[yi]
                            z = np.linspace(self.z_range[0], self.z_range[1], zd)[zi]
                            for bbox in bboxes:
                                # In the cam0 coordinate system
                                cz, cx, cy, l, w, h, ry = bbox
                                cy = - cy
                                cx = - cx
                                # check if (x, y, z) is in the rotated bounding box
                                dx = x - cx
                                dy = y - cy
                                dz = z - cz
                                cos_ry = np.cos(-ry)
                                sin_ry = np.sin(-ry)
                                dx_rot = dx * cos_ry - dz * sin_ry
                                dz_rot = dx * sin_ry + dz * cos_ry
                                if (abs(dx_rot) <= w/2):
                                    if (abs(dy) <= h/2):
                                        if (abs(dz_rot) <= l/2):
                                            is_occupied[yi, zi, xi] = True
                                            is_visible[yi, zi, xi] = False
                is_occupied = is_occupied.reshape(-1)
                is_visible = is_visible.reshape(-1)

        if self.save_gt_occ_map_path != "":
            os.makedirs(self.save_gt_occ_map_path, exist_ok=True)
            if isinstance(dataset, Kitti360Dataset):
                name = "{}_{:010d}".format(seq, dataset._img_ids[seq][init_id])
            elif isinstance(dataset, WaymoDataset):
                name = "{}_{:010d}".format(seq, init_id)
        ...
        is_occupied = is_occupied.reshape(yd, zd, xd)
        is_occupied_pred = is_occupied_pred.reshape(yd, zd, xd)
        is_visible = is_visible.reshape(yd, zd, xd)
        ...
        profiles = render_profile(
            net, 
            self.cfg.CONTROLNET.DATA.VIS_VOLUME,
            self.cfg.CONTROLNET.DATA.CAM_INCL_ADJUST.to(device)
        ).float()
        data["is_occupied"] = is_occupied.any(dim=0)
        data["is_occupied_pred"] = is_occupied_pred.any(dim=0)
        data["profiles"] = profiles
        data["xyz_range_res"] = (self.x_range, self.y_range, self.z_range, is_occupied_pred.size(1), self.y_res, is_occupied_pred.size(1))
...
def get_metrics(config, device):
    ...
    metrics = {}
    for name in names:
        metrics[name] = MeanMetric((lambda n: lambda x: x["output"][n])(name), device)
        metrics[f"{name}_var"] = VarianceMetric((lambda n: lambda x: x["output"][n])(name), device)
    return metrics


def initialize(config: MainConfig, refine_output=True, *args, **kwargs):
    _, _, dataset = make_datasets(config.BTS.DATA)
    return LidarOccupancyWrapper.from_conf(config, dataset=dataset, refine_output=True)
...
def visualize(engine: Engine, logger: TensorboardLogger, output_dir: str, step: int):
    ...
    width, height = (PAGE_WIDTH_INCHES, 4)
    fig, axes = plt.subplots(1, 4, width_ratios=[3, 1, 1, 2], figsize=(width*2, height))

    ax = axes[0]
    ax.imshow(data["imgs"][0][0].permute(1, 2, 0).cpu() * 0.5 + 0.5)
    ax.set(title="Input", xticks=[], yticks=[])

    # We plot the GT and predicted occupancies with norm=False to prevent scaling issues when nothing is occupied. 
    ax = axes[1]
    ax.imshow(color_tensor(1 - data["is_occupied"].float().cpu(), cmap=cmap_magma, norm=False))
    ax.set(title="Lidar Occupancy", xticks=[], yticks=[])
    set_spines(ax, c="green", ls="--", lw=3)
    ax = axes[2]
    ax.imshow(color_tensor(1 - data["is_occupied_pred"].float().cpu(), cmap=cmap_magma, norm=False))
    ax.set(title="Predicted Occupancy", xticks=[], yticks=[])
    set_spines(ax, c="green", ls="--", lw=3)
    
    ax = axes[3]
    plot_profile(
        ax, 
        data["profiles"].squeeze().cpu(), 
        poses=data["out_synth_l"][-1].POSES_IN[0].cpu().float() if "out_synth_l" in data else None, 
        projs=data["data_synth_l"][-1].PROJS[0].cpu().float() if "data_synth_l" in data else None,      # TODO there must be a bug somewhere since data["out_synth_l"][-1].PROJS_IN has the wrong shape
        vis_volume=data["vis_volume"], 
        flip=True
    )
    ax.set(title="Predicted BEV Density")#, xticks=[], yticks=[])
    # set_spines(ax, visible=False)
    x_range, y_range, z_range, x_res, y_res, z_res = data["xyz_range_res"]
    box = Rectangle(
        xy=[x_range[0], z_range[0]], 
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(output_dir, f"{step:04d}.pdf"),
        # dpi=100,
        bbox_inches='tight', 
        pad_inches=0,
    )
    
    writer.add_figure(f"eval/occupancy_bev_{step}", fig, global_step=step, close=True)
