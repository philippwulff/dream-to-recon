    ...
    def __call__(self, data):
        with profiler.record_function("loss_computation"):
            ...
            for scale in range(n_scales):
                ...
                coarse_in = data["coarse_in"][scale]
                fine_in = data["fine_in"][scale]

                rgb_coarse = coarse["rgb"]
                depth_coarse = coarse["depth"]
                depth_coarse_in = coarse_in["depth"]
                rgb_fine = fine["rgb"]
                depth_fine = fine["depth"]
                depth_fine_in = fine_in["depth"]
                rgb_gt = data["rgb_gt"]
                rgb_gt_in = data["rgb_gt_in"]
                ...
                rgb_gt_in = rgb_gt_in.unsqueeze(-2)
                ...
                rgb_loss = self.rgb_coarse_crit(depth_coarse.unsqueeze(-1).unsqueeze(-1), rgb_gt)       # FIXME unsqueeze
                rgb_loss_in = self.rgb_coarse_crit(depth_coarse_in.unsqueeze(-1).unsqueeze(-1), rgb_gt_in)       # FIXME unsqueeze
                
                # Take minimum across all reconstructed views
                rgb_loss = rgb_loss.amin(-2)
                rgb_loss_in = rgb_loss_in.amin(-2)
                ...
                # rgb_loss = torch.zeros_like(rgb_loss.mean())
                rgb_loss = rgb_loss.mean()
                rgb_loss_in = rgb_loss_in.mean()

                loss_coarse_all += rgb_loss.item() * self.lambda_coarse
                loss_coarse_in_all += rgb_loss_in.item() * self.lambda_coarse
                if using_fine:
                    # fine_loss = self.rgb_fine_crit(rgb_fine, rgb_gt)
                    fine_loss = self.rgb_fine_crit(depth_fine.unsqueeze(-1).unsqueeze(-1), rgb_gt)
                    fine_loss = fine_loss.amin(-2)
                    fine_loss_in = self.rgb_fine_crit(depth_fine_in.unsqueeze(-1).unsqueeze(-1), rgb_gt_in)       # FIXME unsqueeze
                    fine_loss_in = fine_loss_in.amin(-2)
                    ...
                    # fine_loss = torch.zeros_like(fine_loss.mean())
                    fine_loss = fine_loss.mean()
                    fine_loss_in = fine_loss_in.mean()
                    
                    rgb_loss = (rgb_loss + rgb_loss_in) * self.lambda_coarse + (fine_loss + fine_loss_in) * self.lambda_fine
                    loss_fine_all += fine_loss.item() * self.lambda_fine
                    loss_fine_in_all += fine_loss_in.item() * self.lambda_fine
                else:
        ...
        loss_dict["loss_rgb_coarse_in"] = loss_coarse_in_all
        loss_dict["loss_rgb_fine"] = loss_fine_all
        loss_dict["loss_rgb_fine_in"] = loss_fine_in_all
