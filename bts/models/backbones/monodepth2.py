...
from bts.models.layers.layers import *
...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1):
        ...
        if num_input_images > 1:
            ...
            net, weights = resnets[num_layers]
            self.encoder = net(weights=weights if pretrained else None)
            
        self.input_transforms = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])

        if num_layers > 34:
            self.num_ch_enc[1:] *= 4

    def forward(self, input_image):
        # TODO do not use .self
        self.features = []
        x = (input_image - 0.45) / 0.225        # TODO replace with input transforms
...
class Decoder(nn.Module):
    def __init__(self, num_ch_enc, num_ch_dec=None, d_out=1, scales=range(4), use_skips=True, norm=None):
        ...
        if num_ch_dec is None:
            self.num_ch_dec = [128, 128, 256, 256, 512]
        else:
        ...
        for i in range(4, -1, -1):
            ...
            num_ch_in = self.num_ch_enc[-1] if i == 4 else self.num_ch_dec[i + 1]
            num_ch_out = self.num_ch_dec[i]
            self.convs[("upconv", i, 0)] = ConvBlock(num_ch_in, num_ch_out, norm=norm)
            ...
            if self.use_skips and i > 0:
                num_ch_in += self.num_ch_enc[i - 1]
            num_ch_out = self.num_ch_dec[i]
            self.convs[("upconv", i, 1)] = ConvBlock(num_ch_in, num_ch_out, norm=norm)
    ...
    def forward(self, input_features):
        with profiler.record_function("encoder_forward"):
            ...
            for i in range(4, -1, -1):
                ...
                # print(f"decoder conv block {i}_0 abs max:", x.abs().max())
                ...
                # Concat skip tensor from encoder to current feats
                if self.use_skips and i > 0:
                ...
                # print(f"decoder conv block {i}_1 abs max:", x.abs().max())

                self.outputs[("features", i)] = x
                
                # TODO maybe regularize this?
                # if x.dtype == torch.float16:
                    # x = x.clamp(torch.finfo(torch.float16).min, torch.finfo(torch.float16).max)
                    # x = x.clamp(- 64704 / 2, 64704)
                    
                # Think about adding normalization layers
                # https://discuss.pytorch.org/t/training-tricks-to-improve-stability-of-mixed-precision/167310

                if torch.isnan(x).any():
                    print("nan in backbone x")
                    pass                    
                if torch.isinf(x).any():
                    print("inf in backbone x")
                    pass                    
                if torch.isnan(self.outputs[("features", i)]).any():
                    print(f"nan in backbone self.outputs[('features', {i})]")
                    pass                    
                if torch.isinf(self.outputs[("features", i)]).any():
                    print(f"inf in backbone self.outputs[('features', {i})]")
                    pass   
                                 
                if i in self.scales:
                    self.outputs[("disp", i)] = self.decoder[self.decoder_keys[("dispconv", i)]](x)
                    if torch.isnan(self.outputs[("disp", i)]).any():
                        print(f"nan in backbone self.outputs[('disp', {i})]")
                        pass                    
                    if torch.isinf(self.outputs[("disp", i)]).any():
                        print(f"inf in backbone self.outputs[('features', {i})]")
                        pass                    
...
class Monodepth2(nn.Module):
    ...
    def __init__(
        ...
        # decoder
        self.decoder = Decoder(num_ch_enc=self.num_ch_enc, d_out=self.d_out, num_ch_dec=num_ch_dec, scales=self.scales, norm=decoder_norm)
    ...
    def forward(self, x):
        ...
        with profiler.record_function("encoder_forward"):
            ...
            if any([torch.isinf(_).any() or torch.isnan(_).any() for _ in image_features]):
                pass
                print("encoder abs max:", max([f.abs().max() for f in image_features]))
    ...
    def from_conf(cls, conf):
        return cls(
            ...
            decoder_norm=conf.get("decoder_norm", None),
        )
